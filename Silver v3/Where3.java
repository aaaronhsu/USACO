...
public class Where3 {
	...
	static int squares(int dimX, int dimY) {
		int hold = 0;

		for (int i = 0; i < n - dimX + 1; i++) {
			for (int a = 0; a < n - dimY + 1; a++) {

				
				if (check(i, a, i + dimX - 1, a + dimY - 1)) {
					hold++;
					addRestrictions(i, a, i + dimX - 1, a + dimY - 1);
					System.out.println(i + ", " + a + " to " + (i + dimX - 1) + ", " + (a + dimY - 1));
				}
			}
		}

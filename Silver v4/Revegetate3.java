...
public class Revegetate3 {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < n; i++) {
			if (!visited[i]) {
				if(dfs(cows, pastures, visited, i, 1)) {
					ans.append("0");
				}
				else {
	...
	static boolean dfs(LinkedList[] cows, int[] pastures, boolean[] visited, int index, int seed) {
		if (visited[index]) {
			if (pastures[index] != seed) return false;
			return true;
		}
		...
		for (Cow c : (LinkedList<Cow>) cows[index]) {
			if (c.type == 1) ret = ret && dfs(cows, pastures, visited, c.nextPasture, seed);
			else ret = ret && dfs(cows, pastures, visited, c.nextPasture, seed * -1);
		}
